...
- S3 POS CSV 로더: 아직 미구현입니다. 현재 API는 `app/data/*.csv` 로컬 샘플을 읽습니다.
...
목표 운영 흐름은 POS 하루 마감 CSV를 S3에 적재하고, 서버가 해당 CSV를 내려받아 학습된 LightGBM 모델로 수요를 예측하는 방식입니다. 다만 현재 코드에는 S3 CSV loader가 아직 구현되어 있지 않습니다.

현재 API는 `app/data/` 파일을 로컬 개발과 테스트용 샘플 CSV로 읽습니다. S3 loader를 붙일 때도 아래와 같은 스키마를 맞추면 같은 전처리/예측 파이프라인을 사용할 수 있습니다.
...
```json
{
  ...
  "gaps": [
    ...
    "API endpoints currently read local app/data CSV files, not S3."
  ]
}
```
...
현재는 로컬 `app/data/inventory_flow_5days.csv` 샘플을 기반으로 수요를 예측하는 엔드포인트입니다. 요청 body는 필요 없습니다.
...
현재 코드에서는 같은 스키마의 샘플 CSV를 `app/data/`에서 읽습니다. S3 loader는 아직 연결되지 않았습니다.
...
- 현재 구현은 로컬 CSV를 읽습니다. S3에서 가져오는 코드는 아직 미구현입니다.
