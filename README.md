...
백엔드가 넘겨준 POS 마감 이력과 grounding 데이터를 기반으로 수요 예측, 발주 추천용 수요 분위 예측, LLM 설명, 챗봇 답변을 제공하는 FastAPI 서버입니다. 점주가 추천 결과를 이해할 수 있도록 로컬 Llama 3.2 1B로 설명 출력과 챗봇 기능을 제공합니다.
...
기존 데모 API(`/forecast`, `/order-recommendation`, `/chat`)가 로컬 CSV 대신 S3 CSV를 직접 읽게 하려면 `.env`를 아래처럼 설정합니다.
...
curl -X POST http://127.0.0.1:8000/v1/chat \
  ...
  -d '{
    ...
    "history":[{"role":"user","content":"우유 발주 추천 설명해줘"}]
  }'
```
...
Spring 백엔드가 붙는 API는 아래 네 개입니다. `/daily-close`는 백엔드에서 실행하므로 AI 서버에서는 제공하지 않습니다. 기존 `/forecast`, `/order-recommendation`, `/chat`은 로컬 데모용 엔드포인트입니다.
...
백엔드가 만든 grounding 숫자를 한 번의 설명 문장으로 바꿉니다. `cacheHit`, `latencyMs`, `tokens`는 항상 포함됩니다. 같은 요청은 Redis 또는 인메모리 exact cache로 재사용합니다. 로컬 Llama 연결에 실패하면 fallback 문장을 반환합니다.
...
- exact cache는 `/forecast`, `/order-recommendation`, `/v1/generate` 계산/생성 결과에 사용됩니다.
- semantic cache는 `/v1/chat`, `/chat`에서 Llama 반복 호출을 줄이는 데만 사용됩니다.
...
- 유사 질문은 semantic cache로 재사용해 Llama 반복 호출을 줄입니다.
- semantic cache namespace에는 store ID, business date, data version이 포함됩니다.

## 캐시 정책

- `/forecast`, `/order-recommendation`, `/v1/generate`: exact cache 사용
- `/v1/chat`, `/chat`: semantic cache 사용
