...
- S3 POS CSV 로더: `DATA_SOURCE=s3`일 때 `inventory_flow`, `item_master`, `order_policy` CSV를 S3에서 읽습니다.
...
기존 데모 API(`/forecast`, `/order-recommendation`, `/daily-close`, `/chat`)가 로컬 CSV 대신 S3 CSV를 읽게 하려면 `.env`를 아래처럼 설정합니다.
...
대상은 위 세 CSV 객체입니다. AWS 인증은 환경변수, `AWS_PROFILE`, EC2/ECS IAM Role 등 boto3 기본 인증 체인을 따릅니다.
